              ...
              <table className="w-full min-w-full text-left border-collapse text-xs table-auto">
                <thead>
                  <tr className="border-b border-border/80 bg-[#141414] text-muted-foreground font-bold uppercase tracking-wider text-[11px] whitespace-nowrap">
                    <th className="py-2.5 px-3 text-center w-24">{isHindi ? 'क्लास कोड' : 'Class Code'}</th>
                    <th className="py-2.5 px-3">{isHindi ? 'क्लास & सेक्शन' : 'Grade & Section'}</th>
                    <th className="py-2.5 px-3">{t('col.subject')}</th>
                    <th className="py-2.5 px-3 text-center">{isHindi ? 'रूम नंबर' : 'Room'}</th>
                    <th className="py-2.5 px-3 text-center">{isHindi ? 'स्टूडेंट्स काउंट' : 'Students'}</th>
                    <th className="py-2.5 px-3 text-center">{isHindi ? 'वीकली पीरियड्स' : 'Weekly Periods'}</th>
                    <th className="py-2.5 px-3 text-center">{isHindi ? 'एवरेज अटेंडेंस' : 'Avg Attendance'}</th>
                    <th className="py-2.5 px-3 text-center w-24">{t('col.status')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/60">
                  {classes.map((cls) => (
                    <tr key={cls.code} className="hover:bg-[#1a1a1a] transition-colors whitespace-nowrap">
                      <td className="py-2.5 px-3 font-mono font-bold text-primary text-xs text-center">{cls.code}</td>
                      ...
                      <td className="py-2.5 px-3 text-muted-foreground font-mono text-xs text-center">{cls.room}</td>
                      <td className="py-2.5 px-3 font-bold text-foreground text-xs text-center">{cls.students} students</td>
                      <td className="py-2.5 px-3 font-mono text-muted-foreground text-xs text-center">{cls.weeklyPeriods} periods/wk</td>
                      <td className="py-2.5 px-3 font-bold text-emerald-400 text-xs text-center">{cls.avgAttendance}</td>
                      <td className="py-2.5 px-3 text-center">
                        <VFBadge variant="success" className="text-xs px-2 py-0.5 rounded-[3px] mx-auto">{cls.status}</VFBadge>
                      </td>
                    </tr>
                  ))}
                </tbody>
